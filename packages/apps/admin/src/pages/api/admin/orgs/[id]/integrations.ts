...
import {
  getDb,
  getOrgIntegrationEditorStateForApp,
  organizations,
  setOrgEnabledIntegrationIds,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const data = await getOrgIntegrationEditorStateForApp(id, appSlug)
  ...
  if (req.method === 'PUT') {
    ...
    if (!appSlug) {
      return res.status(400).json({ ok: false, error: 'appSlug is required (e.g. fintracker)' })
    }
    const raw = (body as { enabledIntegrationIds?: unknown }).enabledIntegrationIds
    if (!Array.isArray(raw) || !raw.every((x) => typeof x === 'string')) {
      return res.status(400).json({ ok: false, error: 'enabledIntegrationIds must be a string array' })
    }
    await setOrgEnabledIntegrationIds(id, raw as string[], appSlug)
    const data = await getOrgIntegrationEditorStateForApp(id, appSlug)
    return res.status(200).json({ ok: true, data })
  }
