...
interface SettingsSection {
  ...
  fields: SettingField<StaffSettings>[]
}
...
const SETTINGS_SECTIONS: SettingsSection[] = [
  {
    ...
    fields: [{ key: 'staffAttendanceSpreadsheetId', label: 'Spreadsheet ID', type: 'text' }],
  },
]
...
export default function StaffSettingsPage() {
  ...
  const validateLiveStatus = useCallback(async () => {
    setLoading(true)
    setError('')
    ...
    try {
      ...
      SETTINGS_SECTIONS.forEach(section => {
        section.fields.forEach(field => {
          loaded[String(field.key)] = String(settingsResult[field.key as keyof StaffSettings] || '')
        })
      })
      setSettings(loaded)
  ...
  async function saveField(key: string, value: string) {
    ...
    setError('')
    try {
      await api.saveSettings(value.trim())
  ...
  const sheetId = settings.staffAttendanceSpreadsheetId?.trim() || ''
